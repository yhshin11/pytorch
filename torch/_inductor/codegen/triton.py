    ...
    def indexing(
        ...
        *,
        ...
        if (
            ...
            def match_strided_block(index: sympy.Expr, range_tree: IterationRangesEntry) -> Optional[BlockPtrOptions]:
                """
            ...
            def match_mod_div_block(index: sympy.Expr, range_tree: IterationRangesEntry) -> Optional[BlockPtrOptions]:
                """
                ...
                num_dims = max(
                    ...
                    (
                        index.count(FloorDiv)
                        + index.count(ModularIndexing)
                        ),
                    )
            ...
            def match_block_pointer_subexpr(expr: sympy.Expr, range_tree: IterationRangesEntry) -> Optional[tuple]:
                """
            ...
            def match_block_pointer() -> Optional[BlockPtrOptions]:
                ...
                for tree in range_trees:
                    ...
                    match = match_block_pointer_subexpr(subexpr, tree)
                    if match is None:
                        return None

                    # Flatten block dimensions.
                    num_dims = len(match[0])
                    block_params += [[param[dim] for param in match] for dim in range(num_dims)]
                ...
                (shape, block_shape, strides, block_offsets) = zip(*block_params)

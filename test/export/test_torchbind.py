...
def forward(self, token, obj_attr, x, n):
    with_effects = torch._higher_order_ops.effects.with_effects(token, torch.ops._higher_order_ops.call_torchbind, obj_attr, 'add_tensor', x);  token = obj_attr = None
...
def forward(self, token, x, cc):
    with_effects = torch._higher_order_ops.effects.with_effects(token, torch.ops._higher_order_ops.call_torchbind, cc, 'add_tensor', x);  token = cc = None
    getitem = with_effects[0]
    getitem_1 = with_effects[1];  with_effects = None
    add = torch.ops.aten.add.Tensor(x, getitem_1);  x = getitem_1 = None
    return (getitem, add)""",  # noqa: B950
